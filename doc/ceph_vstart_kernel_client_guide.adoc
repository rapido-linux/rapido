...
The VMs run the corresponding autorun script (e.g. 'rbd_autorun.sh'
or 'cephfs_autorun.sh') during boot, and then present an interactive
...
'rbd_autorun.sh' will be executed on boot. 'rbd_autorun.sh' performs
the following:
...
'cut_blktests_rbd.sh' (and corresponding 'blktests_rbd_autorun.sh')
...
'cephfs_autorun.sh' will be executed on boot, performing the following:
...
'fstests_cephfs_autorun.sh' boot script additionally:
