...
_rt_require_dracut_args "$RAPIDO_DIR/autorun/lib/fstests.sh" \
			"$RAPIDO_DIR/autorun/fstests_bcachefs.sh" "$@"
_rt_require_fstests
req_inst=()
_rt_require_bcachefs_tools req_inst
...
"$DRACUT" --install "tail blockdev ps rmdir resize dd vim grep find df sha256sum \
	...
	--add-drivers "zram lzo lzo-rle dm-snapshot dm-flakey bcachefs \
		       loop scsi_debug dm-log-writes ext4 virtio_blk" \
	--modules "base" \
	"${DRACUT_RAPIDO_ARGS[@]}" \
